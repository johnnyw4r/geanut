...
if ($nvars > 0 && $nvars <= 50) {
	...
	$ancho=800;
	$margen=120;
	/* Calcula el esacio vacio entre barras */
	$separacion = (($ancho - $margen) - ($ladoy * $nvars))/($nvars);
	//$separacion =;
	//$equis1 = 60;
	if ($separacion >40)
		$equis1 =$separacion;    // equis IZQUIERDA
	else
		$equis1 = 60;
		
	$equis2 = $equis1 + $ladoy;// + $separacion;    // equis DERECHA
	...
	for ($semestre = 0; $semestre < $nvars; $semestre++) {
		...
		imagefilledpolygon($image, $puntos, 4, $colores[$idcolor + 1]); //Se le asigna el color deseado
...
//		ImageString($image,1,$equis1 + 2, 51 + (180 - $porcentaje),$valores[$semestre],$colores[4]);
		...
		$equis1 = $equis1 + $ladoy + $separacion;
		$equis2 = $equis2 + $ladoy + $separacion;
	}
} else {
	/* crea imagen */
